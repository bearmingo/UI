...
PassRefPtr<TransformOperation> ScaleTransformOperation::blend(const TransformOperation* from, double progress, bool blendToIdentity)
{
    ...
    if (blendToIdentity)
        return ScaleTransformOperation::create(UI::blend(m_x, 1.0, progress),
                                               UI::blend(m_y, 1.0, progress),
                                               UI::blend(m_z, 1.0, progress), m_type);
    ...
    return ScaleTransformOperation::create(UI::blend(fromX, m_x, progress),
                                           UI::blend(fromY, m_y, progress),
                                           UI::blend(fromZ, m_z, progress), m_type);
}

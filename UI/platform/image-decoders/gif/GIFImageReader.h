...
struct GIFFrameReader {
    ...
    UI::ImageFrame::FrameDisposalMethod disposal_method;   /* Restore to background, leave in place, etc.*/
    ...
    GIFFrameReader() {
        ...
        tpixel = 0;
        disposal_method = UI::ImageFrame::DisposeNotSpecified;
...
struct GIFImageReader {
    ...
    GIFImageReader(UI::GIFImageDecoder* client = 0) {
        clientptr = client;
        state = gif_type;
        bytes_to_consume = 6;
        bytes_in_hold = 0;
    ...
    bool read(const unsigned char * buf, unsigned int numbytes, 
              UI::GIFImageDecoder::GIFQuery query = UI::GIFImageDecoder::GIFFullQuery, unsigned haltAtFrame = -1);
